...
function initDCV () {
    ...
    if (window.DCVWebRTCPeerConnectionProxy) {
        window.DCVWebRTCPeerConnectionProxy.setInitCallback( function(result) {
            if(result.success) {
                dcvProxy = result.proxy;
                dcvProxy.overrideWebRTC();
                // dcvProxy.disableUnloadCleanup = true;
                console.log('Detect DCV redirection environment success,', dcvProxy.getVersion());
                setTimeout ( () => {
                    call();
                }, 2000);

            } else {
                console.warn('Detect DCV redirection environment failure:', result.error);
            }
        }, MAX_WAIT_MS);
    } else {
        console.warn('Detect DCV redirection environment failure: sdk is not loaded');
     }
}
...
function gotStream(stream) {
    ...
    // setup mic control
    const processedStream  = stream; //setupAudioControls(stream);

    processedStream.getTracks().forEach(track => pc1.addTrack(track, processedStream));
...
function setDCVStream(stream) {
    if (stream && stream.type === 'proxy') {
        const { x, y, width: w, height: h } = audio2.getBoundingClientRect();
        const style = `position: absolute; left: ${x + window.screenLeft}px; top: ${y + window.screenTop}px; width: ${w}px; height: ${h}px;`;
        const props = {
            kind: 'audio',
            style,
            autoplay: true,
        ...
        const remoteAudio = stream.createMediaElement(props);
